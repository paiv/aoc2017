...
class DuetDisasm:
    ...
    def _name_regs(self, image, state):
        rip, rsnd = vm.DuetVM.IP, vm.DuetVM.SND
        rnames = dict(zip(range(len(image.regs)), image.register_names))
        opsra = set('addi addr jgri jgrr jnri jnrr modi modr muli mulr rcv seti setr sndr subi subr'.split())
        opsrb = set('addr jgir jgrr jnir jnrr modr mulr setr sndr subr'.split())

        def name_regs(op, a, b):
            if op in opsra:
                a = 'ip' if a == rip else 'snd' if a == rsnd else f'r{rnames[a]}'
            if op in opsrb:
                b = 'ip' if b == rip else 'snd' if b == rsnd else f'r{rnames[b]}'
            return (op, a, b)
    ...
    def _c_expr(self, image, state):
        def expr(op, a, b):
            ...
            if op in ('addr',):
                return (a, '+=', b)
            elif op in ('jgii', 'jgir', 'jgri', 'jgrr'):
                return (a, 'jg', b)
            elif op in ('jnii', 'jnir', 'jnri', 'jnrr'):
                return (a, 'jnz', b)
            elif op in ('modi', 'modr'):
                return (a, '%=', b)
            elif op in ('muli', 'mulr'):
                return (a, '*=', b)
            elif op in ('seti', 'setr'):
                return (a, '=', b)
            elif op in ('rcv', 'sndi', 'sndr'):
                return (None, op[:3], a)
            elif op in ('subi',):
                return (a, '-=', b) if b >= 0 else (a, '+=', -b)
            elif op in ('subr',):
    ...
    def _expr_tree(self, image, state):
        def expr(sop):
            ...
            if op == '=':
                ...
                if isinstance(b, int):
                    goto = GotoExpr(i + b, f'ln{i + b}')
                else:
                    goto = GotoRExpr(expr(b))
                return IfExpr(cond=BinaryExpr(expr(a), '>', expr(0)), if_true=goto)
            elif op == 'jnz':
                if isinstance(b, int):
                    goto = GotoExpr(i + b, f'ln{i + b}')
                else:
                    goto = GotoRExpr(expr(b))
                return IfExpr(cond=expr(a), if_true=goto)
            elif op == 'rcv':
                return RcvExpr(expr(b))
            elif op == 'snd':
                return SndExpr(expr(b))
            else:
                raise NotImplementedError((i, l, a, op, b))
    ...
    def _else_if(self, image, state):
        ...
        def expr(block):
            ...
            def inner(block):
                ...
                assert if_expr is None, if_expr
                return new_block
    ...
    def _generate(self, image, state, output=None):
        state, labels = state
        regs = dict(zip((f'r{r}' for r in image.register_names), image.regs))
        del regs['rip']
        del regs['rsnd']

        writer = ExprWriter(labels=labels, output=output)
        state._accept(writer)
        return writer.getvalue(), regs
...
class GotoRExpr(Expr):
    def __init__(self, reg):
        super().__init__()
        self.reg = reg

    def __repr__(self):
        return f'GotoR({self.reg})'
    ...
    def _accept(self, visitor):
        return visitor.goto_r_expr(self)
...
class ExprWriter:
    ...
    def goto_r_expr(self, expr):
        self.expr(expr)
        self.write_line(f'goto_r {expr.reg};')
    ...
    def rcv_expr(self, expr):
        self.expr(expr)
        self.write('RCV(')
        expr.var._accept(self)
        self.write_line(');')

    def snd_expr(self, expr):
        self.expr(expr)
        self.write('SND(')
        expr.var._accept(self)
        self.write_line(');')
